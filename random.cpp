...
// -----------------------------------------------------------------------------
//  Standard Cauchy distribution is Cauchy(1, 0), where gamma = 1 and delta = 0
// -----------------------------------------------------------------------------
float cauchy(						// r.v. from Cauchy(gamma, delta)
	float gamma,						// scale factor (gamma > 0)
	float delta)						// location
{
	if (gamma <= 0.0f) error("cauchy() parameters error.\n", true);
	...
	/*
	// -------------------------------------------------------------------------
	//  generate a standard Cauchy(1.0, 0.0) r.v.
	// -------------------------------------------------------------------------
	float g1, g2;
...
// -----------------------------------------------------------------------------
//  Standard Levy distribution is Levy(1, 0), where gamma = 1 and delta = 0
// -----------------------------------------------------------------------------
float levy(							// r.v. from Levy(gamma, delta)
	float gamma,						// scale factor (gamma > 0)
	float delta)						// location
{
	if (gamma <= 0.0f) error("levy() parameters error.\n", true);
...
{
	if (p <= 0.0f || p > 2.0f) {
		error("p_stable() parameters error! p is NOT in (0, 2]\n", true);
	}
	if (zeta < -1.0f || zeta > 1.0f) {
		error("p_stable() parameters error! beta is NOT in [-1, 1]\n", true);
	}
	if (gamma <= 0.0f) {
		error("p_stable() parameters error! gamma <= 0\n", true);
	}
	...
	if (fabs(zeta) < FLOATZERO) {	// symmetric random variable
	...
	else {							// non-symmetric random variable
		if (fabs(p - 1.0f) < FLOATZERO) {
			float t0 = PI / 2.0f;
...
float levy_cdf(						// cdf of Levy(0, 1) in range (0, x]
	...
	float step)							// step increment
{
	if (x <= 0.0f) error("levy_cdf() parameters error.\n", true);
...
// -----------------------------------------------------------------------------
//  Using Monte Carlo method to simulate the original collision probability
// -----------------------------------------------------------------------------
	...
	float& p2)							// p2 = p(w / (c * r)), returned
{
	...
	//  release space
	// -------------------------------------------------------------------------
	if (object != NULL || query1 != NULL || query2 != NULL || a != NULL) {
